...
const els = {
  ...
  rotationAngleBtn: $("rotationAngleBtn"),
...
function bindEvents() {
  ...
  els.rotateLeftBtn.addEventListener("click", () => rotateMapBy(-15));
  els.rotateRightBtn.addEventListener("click", () => rotateMapBy(15));
  els.rotationAngleBtn.addEventListener("click", () => rotateMapTo(0));
...
async function planRoute(overrides = {}) {
  ...
  try {
    ...
    // Navigation begins at the user's active checkpoint. Keep that point in
    // the visual centre instead of fitting the whole route and leaving the
    // start marker near a corner of the screen.
    centerOnCheckpoint(displayTarget || route.checkpoints?.[0]);
...
function enableClockwiseMapRotation(svg) {
  ...
  const userRotationGroup = document.createElementNS(SVG_NS, "g");
  userRotationGroup.id = "campusnav-user-rotation";
  projectionGroup.appendChild(clockwise);
  userRotationGroup.appendChild(projectionGroup);
  svg.appendChild(userRotationGroup);
  state.userRotationGroup = userRotationGroup;
  state.displayBounds = { ...bounds };
  applyUserRotationTransform();
...
function sourcePointToDisplay(x, y) {
  ...
  if (!state.mapRotationDegrees || !state.displayBounds) return projected;
  const bounds = state.displayBounds;
  const cx = bounds.x + bounds.w / 2;
  const cy = bounds.y + bounds.h / 2;
  const radians = state.mapRotationDegrees * Math.PI / 180;
...
function ensureMapMaterialDefs(svg) {
  let defs = svg.querySelector("defs");
  if (!defs) {
    defs = document.createElementNS(SVG_NS, "defs");
    svg.insertBefore(defs, svg.firstChild);
  }
  if (defs.querySelector("#cn-glitter-classroom")) return;

  const gradients = {
    classroom: ["#10234D", "#28558B", "#122957"],
    lab: ["#0C3548", "#16718A", "#10374E"],
    office: ["#183A31", "#3D6B43", "#213B2D"],
    bathroom: ["#08456D", "#128EC5", "#0A4D78"],
    utility: ["#202B49", "#435275", "#242E50"],
    courtyard: ["#0B4938", "#2F9A58", "#10533D"],
    auditorium: ["#38205D", "#7045A5", "#37215C"],
    multipurpose: ["#16284E", "#385A87", "#17284B"],
    lift: ["#4C1C9E", "#A53BE4", "#5A20B2"],
    stair: ["#8C350E", "#F0781F", "#A94111"],
    seating: ["#624018", "#B6792B", "#70481B"],
  };

  const sparkleColors = {
    classroom: "#A9E6FF", lab: "#91F1FF", office: "#D8FFB2",
    bathroom: "#B7F2FF", utility: "#C7D5FF", courtyard: "#C9FFD0",
    auditorium: "#F0C7FF", multipurpose: "#B8E2FF", lift: "#F2C2FF",
    stair: "#FFE0A1", seating: "#FFE2A8",
  };
  const glints = [
    [.14,.20,.007,.82], [.34,.72,.005,.58], [.57,.28,.006,.72],
    [.78,.66,.008,.76], [.88,.18,.004,.52], [.21,.52,.004,.48],
  ];

  for (const [name, colors] of Object.entries(gradients)) {
    const gradient = document.createElementNS(SVG_NS, "linearGradient");
    gradient.id = `cn-gradient-${name}`;
    ...
    base.setAttribute("fill", `url(#cn-gradient-${name})`);
    pattern.appendChild(base);
    for (const [cx, cy, radius, opacity] of glints) {
      ...
      sparkle.setAttribute("fill", sparkleColors[name]);
...
function containedShape(label,shapes,svg,anchor=null) {
  ...
  // Blueprint exports frequently stack a room surface, its outline, and a
  // larger enclosing boundary at the same coordinate. The smallest filled
  // outline is the actual room footprint; rejecting all overlaps left rooms
  // unstyled even though their source geometry was present.
  return candidates.sort((a,b)=>{
    const aBox=a.getBBox(),bBox=b.getBBox();
...
function colorRoomLabels(svg,floorId) {
  ...
  for(const label of labels) {
    ...
    title.setAttribute("class","cn-room-code");
...
function drawCheckpoint(group, cp, index) {
  ...
  const arrivalPop = arrived && state.justCompletedIndex === index;
  ...
  circle.setAttribute(
    "class",
    `${baseClass}${arrivalPop ? " cn-checkpoint-arrival-pop" : ""}${isNext ? " cn-checkpoint-next" : ""}${cp.is_turn ? " cn-checkpoint-turn" : ""}${index === 0 ? " cn-route-start" : ""}`
  );
...
function applyUserRotationTransform() {
  const group = state.userRotationGroup;
  const bounds = state.displayBounds;
  if (!group || !bounds) return;
  const cx = bounds.x + bounds.w / 2;
  const cy = bounds.y + bounds.h / 2;
  const angle = normalizeMapAngle(state.mapRotationDegrees);
  group.setAttribute("transform", angle ? `rotate(${angle} ${cx} ${cy})` : "");
  els.rotationAngleBtn.textContent = `${angle}°`;
  els.rotationAngleBtn.dataset.angle = String(angle);
  els.rotationAngleBtn.setAttribute(
    "aria-label",
    angle ? `Map rotated ${angle} degrees; reset rotation` : "Map rotation is zero degrees"
  );
  const compass = document.querySelector(".compass");
  compass?.style.setProperty("--map-rotation", `${angle}deg`);
  compass?.setAttribute(
    "aria-label",
    angle ? `Map rotated ${angle} degrees; arrow shows north` : "North is up"
  );
}

function normalizeMapAngle(angle) {
  const normalized = ((Number(angle) + 180) % 360 + 360) % 360 - 180;
  return Object.is(normalized, -0) ? 0 : normalized;
}

function rotateMapBy(delta) {
  rotateMapTo(state.mapRotationDegrees + delta);
}

function rotateMapTo(angle) {
  if (!state.currentSvg || !state.userRotationGroup || !state.displayBounds) return;
  const focusedCheckpoint = checkpointDisplayTarget();
  state.mapRotationDegrees = normalizeMapAngle(angle);
  applyUserRotationTransform();

  // Labels and route annotations live in root coordinates to stay readable.
  // Rebuild only those lightweight overlays after the shared map rotation;
  // the architectural SVG itself is never reparsed or reloaded.
  state.currentSvg.querySelector("#campusnav-map-labels")?.remove();
  raiseMapLabels(state.currentSvg);
  drawRouteOverlay();
  state.originalViewBox = mapContentViewBox(state.currentSvg, state.displayBounds);
  if (focusedCheckpoint?.floor_id === state.selectedFloor) {
    centerOnCheckpoint(focusedCheckpoint);
  } else {
    fitMap();
  }
  showToast(
    state.mapRotationDegrees
      ? `Map angle: ${state.mapRotationDegrees}°`
      : "Map rotation reset"
  );
}
...
function centerOnCheckpoint(cp) {
  ...
  const zoom = 2.55;
  let w = state.originalViewBox.w / zoom;
  let h = state.originalViewBox.h / zoom;
  const viewport = els.mapViewport?.getBoundingClientRect();
  const viewportAspect = viewport?.width && viewport?.height
    ? viewport.width / viewport.height
    : w / h;
  // Match the camera to the visible panel before centering. Otherwise SVG's
  // meet-mode letterboxing changes with every diagonal rotation and makes the
  // active checkpoint appear to drift even though its map coordinate is fixed.
  if (w / h > viewportAspect) h = w / viewportAspect;
  else w = h * viewportAspect;
  state.viewBox = {
